...
void SNBSXMemoryPack::ResetProtocol()
{
    ...
    m_uFlashMode = BSX_FLASH_MODE_ARRAY;
    m_uPageIndex = 0;
    memset(m_PageBuffer, 0xFF, sizeof(m_PageBuffer));
...
/* AURORA_BSXSLOT_FLASH_PROTOCOL_V2_20260907: flash-array mutation helpers. */
void SNBSXMemoryPack::ProgramFlashByte(Uint32 uAddr, Uint8 uData)
{
    Uint32 a;
    Uint8 oldValue;
    Uint8 newValue;
    ...
    /* NOR flash programming can only change 1 -> 0 without an erase. */
    newValue = (Uint8)(oldValue & uData);
...
void SNBSXMemoryPack::EraseFlashBlock(Uint32 uAddr)
{
    Uint32 a;
    Uint32 base;
    Uint32 i;
    ...
    a = uAddr & (SNES_BSX_MEMORY_PACK_BYTES - 1);
    base = a & 0x0F0000u; /* 16 x 64 KiB blocks in the 8M pack. */

    for (i = 0; i < 0x10000u; ++i)
    {
        if (m_pData[base + i] != 0xFF)
        {
            changed = TRUE;
            break;
        }
    ...
    if (changed)
    {
        memset(m_pData + base, 0xFF, 0x10000u);
        m_bDirty = TRUE;
        ++m_uBlockEraseCount;
    }
}

void SNBSXMemoryPack::EraseFlashAll()
{
    Uint32 i;
    Bool changed = FALSE;
...
Uint8 SNBSXMemoryPack::Read(Uint32 uAddr)
{
    ...
    switch (m_uFlashMode)
    {
        case BSX_FLASH_MODE_CHIP:
            ++m_uVendorReadCount;
            if (a == 0) return 0xB0; /* Sharp vendor ID. */
            if (a == 1) return 0xA8; /* LH28F800SU device ID low byte. */
            if ((a & 7u) == 2u) return 0x63;
            return 0x20;

        case BSX_FLASH_MODE_PAGE:
            return m_PageBuffer[m_uPageIndex & 1u][a & 0xFFu];

        case BSX_FLASH_MODE_COMPAT_STATUS:
            ++m_uStatusReadCount;
            return 0x80; /* ready */

        case BSX_FLASH_MODE_EXT_STATUS:
            ++m_uStatusReadCount;
            if ((a & 0xFFFFu) == 0x0002u)
                return 0x80; /* block ready; lock bookkeeping not needed here */
            if ((a & 0xFFFFu) == 0x0004u)
                return (Uint8)(0x86u | (m_uPageIndex & 1u));
            return 0x00;

        case BSX_FLASH_MODE_ARRAY:
        default:
            return m_pData[a];
    }
}
...
void SNBSXMemoryPack::Write(Uint32 uAddr, Uint8 uData)
{
    ...
    /* AURORA_BSXSLOT_FLASH_PROTOCOL_V2_20260907
     * Commands that require one or more following bus writes. */
    if (m_uPendingCommand)
    {
        switch (m_uPendingCommand)
        {
            case 0x10:
            case 0x40:
                ProgramFlashByte(a, uData);
                m_uFlashMode = BSX_FLASH_MODE_COMPAT_STATUS;
                m_uPendingCommand = 0;
                m_uPendingStep = 0;
                return;
            ...
            case 0x38:
                if (uData == 0xD0)
                {
                    Uint8 *p = m_PageBuffer[m_uPageIndex & 1u];

                    /* Memory Pack identification page used by Nintendo's
                     * slotted creation software. */
                    p[0x00] = 0x4D; /* 'M' */
                    p[0x02] = 0x50; /* 'P' */
                    p[0x04] = 0x04;
                    p[0x06] = 0x1A; /* Type 1, 8 Mbit */

                    /* Stable synthetic 48-bit serial, same semantic role as
                     * a physical pack's unique BCD serial. */
                    p[0x08] = 0x00;
                    p[0x0A] = 0x01;
                    p[0x0C] = 0x23;
                    p[0x0E] = 0x45;
            ...
            case 0x74:
                m_PageBuffer[m_uPageIndex & 1u][a & 0xFFu] = uData;
                m_uPendingCommand = 0;
                m_uPendingStep = 0;
                return;
            ...
            case 0x0C:
                if (m_uPendingStep == 0)
                {
                    m_uPendingData = uData;
                    m_uPendingAddrOdd = (Uint8)(a & 1u);
                    m_uPendingStep = 1;
                    return;
                }
                else
                {
                    ...
                    if (m_uPendingAddrOdd)
                        count = (Uint16)uData | ((Uint16)m_uPendingData << 8);
                    else
                        count = (Uint16)m_uPendingData | ((Uint16)uData << 8);

                    /* Count is N-1: commit count+1 bytes from the currently
                     * selected 256-byte page buffer, wrapping its low byte. */
                    for (i = 0; i <= (Uint32)count; ++i)
                    {
                        Uint32 dst = (a + i) &
                            (SNES_BSX_MEMORY_PACK_BYTES - 1);
                        Uint8 value =
                            m_PageBuffer[m_uPageIndex & 1u][dst & 0xFFu];
                        ProgramFlashByte(dst, value);
                    }
            ...
            case 0xE0:
                if (m_uPendingStep == 0)
                {
                    m_uPendingData = uData; /* count low */
                    m_uPendingStep = 1;
                    return;
                }
                if (m_uPendingStep == 1)
                {
                    m_uPendingCount =
                        (Uint16)m_uPendingData | ((Uint16)uData << 8);
                    m_uPendingStep = 2;
                    return;
                }
            ...
            case 0xFB:
                ...
                else
                {
                    ...
                    ProgramFlashByte(a ^ 0u, lo);
                    ProgramFlashByte(a ^ 1u, hi);
                    m_uFlashMode = BSX_FLASH_MODE_COMPAT_STATUS;
                    m_uPendingCommand = 0;
                    m_uPendingStep = 0;
                    return;
            ...
            case 0x97: /* refresh lock status */
            case 0xB0: /* erase suspend/resume */
                if (uData == 0xD0 && m_uPendingCommand == 0xB0)
                    m_uFlashMode = BSX_FLASH_MODE_COMPAT_STATUS;
                m_uPendingCommand = 0;
                m_uPendingStep = 0;
                return;

            case 0x96: /* ready/busy mode */
                m_uPendingCommand = 0;
                m_uPendingStep = 0;
                return;
            ...
            case 0x99:
                if (uData == 0xD0)
                {
                    ...
                    /* Minimal deterministic erase-information table for the
                     * sixteen 64-KiB blocks of an 8-Mbit pack. */
                    for (id = 0; id < 16u; ++id)
                    {
                        Uint32 pos =
                            (id & 3u) * 0x08u +
                            ((id >> 2) & 3u) * 0x40u;
                        Uint32 erased = 0x80000001u;
                        p[(pos + 0u) & 0xFFu] = (Uint8)(erased >> 0);
                        p[(pos + 1u) & 0xFFu] = (Uint8)(erased >> 8);
                        p[(pos + 2u) & 0xFFu] = (Uint8)(erased >> 16);
                        p[(pos + 3u) & 0xFFu] = (Uint8)(erased >> 24);
                    }
                    m_uPageIndex ^= 1u;
                }
                m_uPendingCommand = 0;
    ...
    switch (uData)
    {
        ...
        case 0x71:
            m_uFlashMode = BSX_FLASH_MODE_EXT_STATUS;
            return;
        ...
        case 0x75:
            /* 75h reads the active page buffer.  It is not the vendor-info
             * command; chip identifiers are selected by 90h. */
            m_uFlashMode = BSX_FLASH_MODE_PAGE;
            return;

        case 0x80: /* abort/sleep side effect not timing-critical here */
            return;

        case 0x90:
            m_uFlashMode = BSX_FLASH_MODE_CHIP;
            return;

        case 0xF0: /* sleep; ignored by the synchronous model */
            return;
        ...
        default:
            /* Unknown command: real hardware drops the short queue; it does
             * not program the flash array as ordinary RAM. */
            return;
    }
}
...
void SnesSystem::Reset()
{
	...
	if (m_SA1.IsActive())
	{
	     ...
	     * Rebuild the S-CPU's materialized SA-1 view after reset. */
	    /* AURORA_V12_SELF_AUDIT_GBC_FX1_20260910: audited S-CPU reset-map barrier. */
