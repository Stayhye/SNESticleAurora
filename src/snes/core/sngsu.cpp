...
Uint8 SNGSU::CodeRead(Uint16 pc)
{
    ...
    if (cacheOffset < 512)
    {
        ...
        if (!bWasValid)
        {
            ...
            if (bCodeFromRam)
            {
                /* RAM reads retain their own timing and wrapping path. */
                for (i = 0; i < 16; ++i)
            ...
            else
            {
                ...
                /* AURORA_V12_SELF_AUDIT_GBC_FX1_20260910
                 * Collapse only HOST clock-accounting work: legacy charged
                 * exactly one external access for each of these 16 bytes. */
                ChargeClocks(MemoryClockCost() * 16);

                if (m_PBR >= 0x60)
                {
                    /* Non-RAM banks >= $60 are open/unmapped in this core. */
                    memset(m_Cache + lineBase, 0xFF, 16);
                    bFilled = TRUE;
                }
                else if (m_pRom && m_uRomSize >= 16 &&
                         (m_uRomSize & m_uRomMask) == 0)
                {
                    Uint32 off = RomOffset(m_PBR, base);
                    Uint32 phys = off & m_uRomMask;
                    Bool bBusContiguous = (m_PBR < 0x40)
                        ? (((Uint32)base & 0x7FFFu) <= 0x7FF0u)
                        : ((Uint32)base <= 0xFFF0u);

                    /* No GSU-bus or physical-ROM mirror may occur in-span. */
                    if (bBusContiguous && phys + 16u <= m_uRomSize)
                    {
                        memcpy(m_Cache + lineBase, m_pRom + phys, 16);
                        bFilled = TRUE;
                    }
                }

                /* Boundary crossings / irregular ROMs keep exact old decode. */
                if (!bFilled)
                {
                    for (i = 0; i < 16; ++i)
                        m_Cache[lineBase + i] = RawCodeRead((Uint16)(base + i));
...
void SNGSU::PixFlush(Int32 nCache)
{
    ...
    {
        /* AURORA_V12_SELF_AUDIT_GBC_FX1_20260910
         * Keep exactly one emulated RAM read + write per bitplane, but merge
         * all eight pixel bits at once instead of branching per pixel. */
        const Uint8 writeMask = (Uint8)(
            ((flags & 0x01u) << 7) | ((flags & 0x02u) << 5) |
            ((flags & 0x04u) << 3) | ((flags & 0x08u) << 1) |
            ((flags & 0x10u) >> 1) | ((flags & 0x20u) >> 3) |
            ((flags & 0x40u) >> 5) | ((flags & 0x80u) >> 7));
        ...
        for (Int32 b = 0; b < bpp; b++)
        {
            ...
            byte = (Uint8)((byte & (Uint8)~writeMask) | (plane & writeMask));
            RamWriteByte(addr, byte);
        }
    }
