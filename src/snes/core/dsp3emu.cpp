 ...
 * DSP-3 HLE adapted for SNESticle Aurora from xyproto/zsnes
 * commit 5d79c3213fcb374f67765a409c8a61a002a79583,
 * chips/dsp3emu.c, Git blob 9df81f9b1e161a088b048e47a9e742005620b63c.
 * GNU GPL version 2; see LICENSES/ZSNES-DSP3-GPL-2.0.txt.
 * Adaptation is mechanical: C++ compilation + Aurora bus declarations.
 */
#include "dsp3emu.h"
#include <stdlib.h> /* AURORA_DSP3_LAZY_WORK_V7_1_20260916 */
#include <string.h>
#include <stdint.h>
...
/* AURORA_DSP3_LAZY_WORK_V7_1_20260916
 * DSP-3 OP1E scratch is cold hardware state: keep it out of permanent EE BSS
 * and allocate it only when a real DSP-3 cartridge is selected. */
static int16_t *s_dsp3_op1e_work = NULL;
int16_t *op1e_terrain = NULL;
int16_t *op1e_cost = NULL;
int16_t *op1e_weight = NULL;

int DSP3EnsureWork(void)
{
    if (s_dsp3_op1e_work) return 1;
    s_dsp3_op1e_work = (int16_t *)malloc((size_t)0x6000u * sizeof(int16_t));
    if (!s_dsp3_op1e_work) return 0;
    memset(s_dsp3_op1e_work, 0, (size_t)0x6000u * sizeof(int16_t));
    op1e_terrain = s_dsp3_op1e_work;
    op1e_cost = s_dsp3_op1e_work + 0x2000;
    op1e_weight = s_dsp3_op1e_work + 0x4000;
    return 1;
}

void DSP3ReleaseWork(void)
{
    if (s_dsp3_op1e_work) free(s_dsp3_op1e_work);
    s_dsp3_op1e_work = NULL;
    op1e_terrain = NULL;
    op1e_cost = NULL;
    op1e_weight = NULL;
}
...
void DSP3_OP1E_B(void)
{
    ...
    while (op1e_lcv_turns) {
        ...
        while (op1e_lcv_steps) {
            ...
            if (0 <= op1e_y && op1e_y < DSP3_WinHi && 0 <= op1e_x && op1e_x < DSP3_WinLo) {
                DSP3_DR = (uint8_t)(op1e_x) | ((uint8_t)(op1e_y) << 8);
                DSP3_OP03();
