...
static Int32 g_GameBoyMode = 0; /* AURORA_GB_MODE_GBC_SGB1_SGB2_R8_20260909: default GBC */

Int32 VideoGetSgbBiosModel(void)
{
    /* AURORA_V12_SELF_AUDIT_GBC_FX1_20260910: SGB frontend disabled; API retained. */
    return 0;
}

Int32 VideoGetGameBoyMode(void)
{
    /* AURORA_V12_SELF_AUDIT_GBC_FX1_20260910: public Game Boy policy is permanently GBC. */
    return 0;
}
...
void VideoSettingsLoad(void)
{
	    ...
	    header.magic == VIDEOCFG_MAGIC)
	{
		...
		{
			/* AURORA_GB_MODE_GBC_SGB1_SGB2_R8_20260909: v46 had SGB model but no standalone selector. */
			loaded = MemCardReadFile(path, (Uint8 *)&cfg, VIDEOCFG_V46_BYTES);
			if (loaded)
			{
				cfg.gameboymode = cfg.sgbbios ? 2 : 1;
				cfg.version = VIDEOCFG_VERSION;
			}
		}
...
if (loaded && header.version != VIDEOCFG_VERSION && header.version != 49)
{
	...
	if (loaded && cfg.magic == VIDEOCFG_MAGIC)
	{
		...
		/* AURORA_V12_SELF_AUDIT_GBC_FX1_20260910: ignore legacy SGB policy on load. */
		g_GameBoyMode = 0;
		g_SgbBiosModel = 0;
...
void CVideoScreen::Input(Uint32 buttons, Uint32 trigger)
{
	int dir = 0;
#if AURORA_RUNTIME_TRACE
	/* AURORA_RUNTIME_DEBUGGER_MENU_V5_20260919: only trace-capable builds gain the extra Performance row. */
	const int perfHi = 38;
#else
	const int perfHi = 37;
#endif
